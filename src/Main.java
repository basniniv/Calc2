...
public class Main {
    ...
    public static String calc(String input) throws IOException {
        ...
        System.out.println("Введите арефметическое выражение из двух чисел от 1 до 10 типа: a + b, a - b, a * b, a / b , арабскими или римскими цифрами");
        String in = sc.nextLine();

        if (!in.contains("+") && !in.contains("-")
                && !in.contains("*") && !in.contains("/")) {
            throw new IOException("не является математической операцией"); //Если нет нужного оператора выбросить исключение
        }
        ...
        if (in.length() - in.replaceAll(" ", "").length() > 2) { //если в строке больше двух пробелов после замены регульрного выражения на такое же с пробелом
...
//        System.out.println(in);
        ...
        if ((RomanNumeral.contains((part1.toUpperCase())) && !RomanNumeral.contains(part3.toUpperCase()))){ //Если только первый операнд римская цифра
            throw new IOException("Калькулятор может работать только с римскими или только с арабскими цифрами");
        }

        if ((!RomanNumeral.contains((part1.toUpperCase())) && RomanNumeral.contains(part3.toUpperCase()))){//Если только второй операнд римская цифра
            throw new IOException("Калькулятор может работать только с римскими  или только арабскими цифрами");
        }

        if (part1.length()>=4 && !RomanNumeral.contains((part1.toUpperCase()))){ //Если операнд больше 3 символов и его нет в енам
            throw new IOException("Число должно быть не больше 10");
        }
        if (part3.length()>=4 && !RomanNumeral.contains((part1.toUpperCase()))){ //Если операнд больше 3 символов и его нет в енам
            throw new IOException("Число должно быть не больше 10");
        }

        if ((RomanNumeral.contains((part1.toUpperCase())) && RomanNumeral.contains(part3.toUpperCase())))  { //Если оба операнда содержит только римские цифры то:
